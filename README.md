...
Exactness needs a bound, and the body limit is not one: converting a bignum back to decimal is quadratic, so a single 1 MB integer literal cost 24 s of CPU where 1 MB of text costs 1 ms. Integers past 1000 digits are rejected, which caps a full-size body under 50 ms measured and leaves every realistic value exact. The same bound applies to what `/decrypt` emits, not only to what the service accepts: the request-side check sees a ciphertext as a string, so a decrypted value whose plaintext exceeds the bound is left as it arrived, and an unbounded integer can never reach the response encoder (`router_test.exs`).
...
**The AES key is 32 raw bytes, checked at boot.** `ENCRYPTION_KEY` must be exactly 32 bytes, base64-encoded (`openssl rand -base64 32`); a malformed value refuses the boot with the same kind of message as a missing `SIGNING_SECRET`. Absence is a normal start under the default base64 cipher, and refuses the boot under AES, so the swap cannot leave the service running without the key it needs. The cipher is deliberately minimal: keyed, non-deterministic and authenticated, and real deployment key management, derivation from passphrases and rotation included, is out of scope. The key stays separate from `SIGNING_SECRET`, since a value used to encrypt must not be the value used to sign (`aes_gcm_vector_test.exs`).
