defmodule RiotTakeHome.Cipher.AesGcm do
  ...
  the same plaintext encrypts to a different marker every time. The key is the
  32 bytes that `ENCRYPTION_KEY` carries base64-encoded, decoded and checked at
  boot in `config/runtime.exs`. It is distinct from the signing secret, since a
